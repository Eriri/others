...
The parameter $a_i^k$ represents the sensitivity of the $i$-th attribute to the distance at the position $x_i^k$. When the distance between the rule attribute information and the input data remains unchanged,
the larger the parameter a, the closer the attribute matching degree is to zero.
...
The two activation weight distributions shown in Figure 1 and Figure 2 can be obtained.
\Figure[!t](topskip=0pt, botskip=0pt, midskip=0pt)[width=3.3 in]{Figure_1.png}
{activation weight calculated by traditional methods}
\Figure[!t](topskip=0pt, botskip=0pt, midskip=0pt)[width=3.3 in]{Figure_2.png}
{activation weight calculated by Gaussian function methods}
...
\quad 2) Introduce exponential normalization function for preprocessing to avoid specific constraints during parameter training

\quad 3) Introduce the stochastic gradient descent method using momentum optimization


\subsection{Partial derivative of the parameters of the brb system}
The reasoning process of the improved BRB system is shown in Figure 3.
...
{Inference system operation process}
...
$$\frac{dloss}{d\beta_j^k}=\sum_{i=1}^N\sum_{j=1}^N\frac{dLoss(\beta_i)}{d\beta_i}\frac{d\beta_i}{d\overline{\beta}_j}\frac{d\overline{\beta}_j}{d\beta_j^k}$$
$$\frac{dloss}{dx_l^k}=\sum_{i=1}^N\sum_{j=1}^N\frac{dLoss(\beta_i)}{d\beta_i}\frac{d\beta_i}{d\overline{\beta}_j}\frac{d\overline{\beta}_j}{dw_k}\frac{dw_k}{dx_l^k}$$
$$\frac{dloss}{da_l^k}=\sum_{i=1}^N\sum_{j=1}^N\frac{dLoss(\beta_i)}{d\beta_i}\frac{d\beta_i}{d\overline{\beta}_j}\frac{d\overline{\beta}_j}{dw_k}\frac{dw_k}{da_l^k}$$
...
As mentioned above, the new gradient parameter training model uses an exponential normalization function to preprocess the constrained parameters while meeting the constraint conditions and has no restrictions on the step size.
In order to improve the speed of training, we use momentum optimization stochastic gradient descent method.
