...
\begin{abstract}
    ...
    Since the calculation of the individual matching degree in the inference process needs to set the candidate value of the antecedent attribute parameter,
    ...
    avoid the zero activation problem caused by the non-adjacent candidate values of the antecedent attributes.
    ...
    This simplifies the calculation of rule weights in the inference process and enables the partial derivatives of the parameters of the inference system to be easily constructed.
...
and set the distance-sensitive parameter $a$ of each attribute to be $0.5$.
...
the activation weight of this rule is zero.
If the activation weight of all rules is zero, the inference cannot be performed.
However, the input data in Fig. \ref{fig2} smoothly drops close to zero according to the distance from the rule and will not take a value of zero.
...
The reasoning process of the improved BRB system is shown in Fig. \ref{fig3}.
...
\quad 1): Using the method of combining attribute distancesensitive parameters and Gaussian function instead of
