...
\begin{abstract}
    ...
    it is likely to cause the rule zero activation problem, and the intelligent algorithm for parameter training is not as efficient and accurate as of the gradient method,
    but the partial derivative of the inference system is difficult to construct.
    This paper proposes a new belief rule structure and its gradient training method,
    aiming to solve the rule zero activation problem during the inference process and improve inference accuracy.
    Use the Gaussian function to calculate the similarity of each attribute of the new rule structure instead of the original method.
    The characteristics of the Gaussian function avoid the zero activation problem caused by the attribute reference value set in the original method.
    Based on the new method, the corresponding distance-sensitive parameter is set for each attribute, and the weight parameter of each rule is discarded.
    ...
    the new BRB system trained by stochastic gradient descent with momentum (SGDM-BRB) has a better performance compared with other conventional methods.
    The experimental results show that the SGDM-BRB has higher accuracy and faster training speed than the conventional method.
...
\quad 2) Setting the global weight parameters for antecedent attributes is canceled,
...
\quad 3) A normalized exponential function is proposed to preprocess the restricted parameters to
...
\quad 2) Proving the differentiability of the new BRB system.
...
\begin{thebibliography}{00}
    ...
    \bibitem{a11} R. Chang, S. Zhang, ``An Algorithm for Training Parameters in Belief Rule-bases Based on
    ...
    \bibitem{a16} J. Liu, L. Martinez, A. C. Calzada, ``A novel belief rule base representation, generation and its inference methodology,''
    \emph{Knowledge-Based Systems},
    2013(53):129-141.
    ...
    \bibitem{a26} Y. Wang, ``Parameter learning for an intuitionistic fuzzy belief rule based
    ...
    \bibitem{a34} L. H. Yang, Y. Wang M, J. Liu, L. Mart´ınez, ``A joint optimization method on parameter and structure for belief-rule based systems,''
